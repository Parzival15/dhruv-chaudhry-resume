...
export const experience = [
  {
    ...
    points: [
      "Founding AI and full stack engineer for a large-scale automated internal regression-testing platform that UnitedHealth Group's contact-center operations rely on; used by 45 teams across the organization. Additionally built an adoption dashboard tracking usage and impact metrics, and presented it to leaders across the organization to report progress and drive tool adoption.",
      'Served as team lead delegate which included delegating and coordinating workstreams, unblocking engineers, and representing the team in leadership discussions.',
      "Owned the platform's generative AI strategy end to end by architecting and deploying GenAI agents into production workflows that automate manual work at scale, cutting operational effort by ~45%.",
      'Designed and built REST APIs and microservices secured with OAuth2 and role-based access control; drove application security and DevSecOps through continuous scanning and vulnerability remediation.',
      'Independently designed and delivered multiple live generative-AI training to 30+ engineers and product managers, pairing hands-on demos with a written guide and post-session assessments to measure comprehension and accelerate responsible AI adoption.',
  ...
  },
  {
    ...
    points: [
      'Co-authored Clinicians-in-the-loop: A Narrative Review of Clinician-Facing Integrated AI Within Mental Health Workflows (In Review): conducted subject matter expert interviews and analyzed 12 HITL-AI tools in clinician mental health workflows for decision support, training, and ethical integration.',
      'Led building efforts for a Streamlit RAG application integrating GPT 4 with a ChromaDB vector store to retrieve from a clinical document corpus, letting clinicians query patient issues through an LLM for decision support through prototyping a working human-in-the-loop AI tool alongside the review. Demoed to clinicians and faculty from external institutions (incl. Georgia State and Carnegie Mellon), gathering feedback to iterate on the tool over time and tracking adoption to validate real-world usefulness.',
  ...
  },
  {
    ...
    points: [
      "Built full-stack features for Sukhi's B2B employee mental-health platform (React.js, Python, PHP, SQL) - a digital wellness product serving enterprise clients. Developed custom frontend components and backend logic while collaborating cross-functionally with UX designers and product managers on the Figma-based design.",
    ],
...
export const projects = [
  {
    ...
    points: [
      'Rapidly prototyped and shipped a cross-platform social movie-ranking app end to end in Cursor (React Native/Expo, Next.js 14, TypeScript, Supabase/Postgres), taking it solo from concept to an installable PWA deployed on Vercel.',
      'Onboarded 34 users through a live product demo, collecting real usage and feedback to iterate on features and validate adoption.',
      'Integrated the OpenAI API to power custom movie likeability scores which can be combined between two users to generate combined genre preference, and generated shareable AI mood-board images server-side via Cloudflare Workers AI (FLUX.1-schnell).',
      'Integrated Airtable via its API as a product-ops backend for tracking feature delivery and roadmap status, and organizing user-feedback data to surface adoption trends and prioritize improvements.',
      'Engineered a binary-search ranking engine ("which did you like more?") backed by a Postgres RPC converting subjective picks into consistent 0-10 scores per sentiment band.',
